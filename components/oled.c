/*
 * forked by Gary Han on 2022-04-18
 * @file         oled.c
...
int oled_write_cmd(uint8_t command)
{
    ...
    ret = i2c_master_cmd_begin(I2C_OLED_MASTER_NUM, cmd, 100 / portTICK_PERIOD_MS);
...
int oled_write_long_data(uint8_t *data, uint16_t len)
{
    ...
    ret = i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(I2C_OLED_MASTER_NUM, cmd, 10000 / portTICK_PERIOD_MS);
    i2c_cmd_link_delete(cmd);
    if (ret != ESP_OK)
    {
        return ret;
